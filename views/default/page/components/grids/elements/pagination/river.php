...
if (!$limit = (int) elgg_extract('limit', $getter_options, HYPEALIVE_RIVER_LIMIT)) {
	$limit = get_input($limit_key);
}

$count = (int) elgg_extract('count', $vars, 0);

$next_limit = HYPEALIVE_RIVER_LOAD_LIMIT;
...
if (($offset == 0 && $count <= $limit) || ($offset > 0 && $count <= $offset + $limit)) {
	...
	if ((HYPEALIVE_RIVER_ORDER == 'asc' && HYPEALIVE_RIVER_LOAD_STYLE == 'load_older')
			|| (HYPEALIVE_RIVER_ORDER == 'desc' && HYPEALIVE_RIVER_LOAD_STYLE == 'load_newer')) {
		$nav = "previous";
	} else {
		$nav = "next";
	}
	...
	if ($count - $offset - $limit <= $next_limit) {
		$text = elgg_echo("hj:alive:river:load:$nav:remaining", array($count - $offset - $limit));
	} else {
		$text = elgg_echo("hj:alive:river:load:$nav", array($next_limit));
	}
