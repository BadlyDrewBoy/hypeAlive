...
if (!$limit = (int) elgg_extract('limit', $getter_options, HYPEALIVE_FORUM_COMMENTS_LIMIT)) {
	$limit = get_input($limit_key);
}

$count = (int) elgg_extract('count', $vars, 0);

$next_limit = HYPEALIVE_FORUM_COMMENTS_LOAD_LIMIT;
...
if (($offset == 0 && $count <= $limit) || ($offset > 0 && $count <= $offset + $limit)) {
	...
	if ((HYPEALIVE_FORUM_COMMENTS_ORDER == 'asc' && HYPEALIVE_FORUM_COMMENTS_LOAD_STYLE == 'load_older')
			|| (HYPEALIVE_FORUM_COMMENTS_ORDER == 'desc' && HYPEALIVE_FORUM_COMMENTS_LOAD_STYLE == 'load_newer')) {
		$nav = "previous";
	} else {
		$nav = "next";
	}
	...
	if ($count - $offset - $limit <= $next_limit) {
		$text = elgg_echo("hj:alive:discussions:load:$nav:remaining", array($count - $offset - $limit));
	} else {
		$text = elgg_echo("hj:alive:discussions:load:$nav", array($next_limit));
	}
